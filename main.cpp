...
int StopBrightness = 20;
int StopBrightnessMax = 255;
int ReverseBrightness = 255;
...
// break lights
unsigned long BreakLightTime = 3000;
unsigned long BreakLightPreviousTime;
unsigned long BreakLightCurrentTime;
...
void loop()
{
    ...
    // reverse light
    if (Lights_ONOFF == ON && ((odczytX >= 0 && odczytX <= 20) || (odczytX >= 0 && odczytX <= 20 && odczytY >= 0 && odczytY <= 20)))
    {
        Reverse = true;
    }
    else
        Reverse = false;
    // if (Lights_ONOFF == ON && odczytX >= 0 && odczytX <= 20 && odczytY >= 0 && odczytY <= 20){}
    // reverse light

    // break lights
    if (Lights_ONOFF == ON && odczytX >= 520 && odczytX <= 530 && odczytY >= 520 && odczytY <= 530)
    {
        Neutral = true;
    }
    else
    ...
    // break lights
    if (Stop == true && Neutral == true)
    {
        BreakLightCurrentTime = millis();
        if (BreakLightCurrentTime - BreakLightPreviousTime <= BreakLightTime)
        {
            Break = true;
        }
        else
        ...
        if (Break == true)
        {
            analogWrite(StopLight, StopBrightnessMax);
        }
        else
            analogWrite(StopLight, StopBrightness);
    }
    else
        BreakLightPreviousTime = BreakLightCurrentTime;
    // break lights

    // reverse lights
    if (Reverse == true)
    {
        analogWrite(ReverseLight, ReverseBrightness);
    }
    else
        //reverse lights
        analogWrite(ReverseLight, LOW);
    // blinker left
    ...
    if (Lights_ONOFF == true && Police == true)
    {
        ...
        if (millis() - czasDoZmiany >= przerwaPolicja)
        { // zmiana między świeceniem jednej a drugiej led
            stanNiebieska = LOW;
            stanCzerwona = LOW;
